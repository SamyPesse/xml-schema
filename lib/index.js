...
function applySchema(el, schema, value) {
    ...
    if (schema.text && isBasicValue(innerValue)) {
        if (schema.raw) el.raw(innerValue);
...
// Create a new xml from a schema
function create(schema, value, options) {
    ...
    var xml = builder.create(schema.tag, {
        version: options.version,
        encoding: options.encoding,
        standalone: options.standalone
    });

    applySchema(xml, schema, value || {});
